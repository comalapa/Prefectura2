        ...
        if(txtNombeEsc.getText().toString().trim().length() == 0){
            ...
            if(txtClaveEsc.getText().toString().trim().length() == 0){
                ...
                try {                    
                    ...
                    String sql = "UPDATE tbl_config SET min_retardo="+retardo+", min_falta="+falta+", nombre_escuela='" +txtNombeEsc.getText()+"', clave_escuela='"+txtClaveEsc.getText()+"', nombre_director='"+txtNombreDir.getText()+"';";
                    PreparedStatement stat = c.prepareStatement(sql);
                    
                    stat.execute();
                    
                    if(!rutaLogo.equals("")){
                        File logoEsc = new File(rutaLogo);
                        int tamLogoEsc = (int) logoEsc.length();
                        try {
                            FileInputStream streamImagen = new FileInputStream(logoEsc);
                            PreparedStatement stm = c.prepareStatement("UPDATE tbl_config SET logo_escuela =?");
                            stm.setBinaryStream(1, streamImagen,tamLogoEsc);
                            stm.execute();
                    ...
                    if(!rutaLogoIzq.equals("")){
                        File logoEscIzq = new File(rutaLogoIzq);
                        int tamLogoEscIzq = (int) logoEscIzq.length();
                        try {
                            FileInputStream streamImagen = new FileInputStream(logoEscIzq);
                            PreparedStatement stm = c.prepareStatement("UPDATE tbl_config SET logo_rep_izq =?");
                            stm.setBinaryStream(1, streamImagen,tamLogoEscIzq);
                            stm.execute();
                    ...
                    if(!rutaLogoDer.equals("")){
                        File logoEscDer = new File(rutaLogoDer);
                        int tamLogoEscDer = (int) logoEscDer.length();
                        try {
                            FileInputStream streamImagen = new FileInputStream(logoEscDer);
                            PreparedStatement stm = c.prepareStatement("UPDATE tbl_config SET logo_rep_der =?");
                            stm.setBinaryStream(1, streamImagen,tamLogoEscDer);
                            stm.execute();
